"""
Local Grounding DINO Candidate Annotation Generator (Pilot Test: 10 Images)
==========================================================================
Generates candidate bounding boxes for turmeric leaf disease detection using
local IDEA-Research/grounding-dino-tiny on CPU.

Outputs:
  - D:\\curuma\\turmeric_datasets\\auto_annotations\\images\\train\\
  - D:\\curuma\\turmeric_datasets\\auto_annotations\\labels\\train\\
  - D:\\curuma\\turmeric_datasets\\auto_annotations\\audit\\pilot_audit.csv
  - D:\\curuma\\turmeric_datasets\\auto_annotations\\audit\\pilot_contact_sheet.png
"""
...
# 10 test images
PILOT_IMAGES = [
    # Aphids_Disease (Class ID: 0)
    {"file": "aphids_disease_(50).jpg", "class_name": "Aphids_Disease", "class_id": 0, "prompt": "aphids . aphid colony ."},
    {"file": "aphids_disease_(59).jpg", "class_name": "Aphids_Disease", "class_id": 0, "prompt": "aphids . aphid colony ."},
    {"file": "aphids_disease_(100).jpg", "class_name": "Aphids_Disease", "class_id": 0, "prompt": "aphids . aphid colony ."},
    # Blotch (Class ID: 1)
    {"file": "blotch_(72).jpg", "class_name": "Blotch", "class_id": 1, "prompt": "leaf blotch . blotch disease ."},
    {"file": "blotch_(7).jpg", "class_name": "Blotch", "class_id": 1, "prompt": "leaf blotch . blotch disease ."},
    {"file": "blotch_(119).jpg", "class_name": "Blotch", "class_id": 1, "prompt": "leaf blotch . blotch disease ."},
    # Leaf_Spot (Class ID: 2)
    {"file": "leaf_spot_(182).jpg", "class_name": "Leaf_Spot", "class_id": 2, "prompt": "leaf spot . fungal spot ."},
    {"file": "leaf_spot_(79).jpg", "class_name": "Leaf_Spot", "class_id": 2, "prompt": "leaf spot . fungal spot ."},
    {"file": "leaf_spot_(63).jpg", "class_name": "Leaf_Spot", "class_id": 2, "prompt": "leaf spot . fungal spot ."},
    # Healthy_Leaf (Negative background: 0 boxes)
    {"file": "healthy_(100).jpg", "class_name": "Healthy_Leaf", "class_id": None, "prompt": None},
]

def apply_nms(boxes, scores, iou_thresh=0.45):
    """Simple Non-Maximum Suppression on xyxy boxes."""
    if len(boxes) == 0:
        return []
    x1 = boxes[:, 0]
    y1 = boxes[:, 1]
...
def main():
    print("=" * 70)
    print("RUNNING LOCAL GROUNDING DINO CANDIDATE ANNOTATION (10-IMAGE PILOT)")
    print("=" * 70)
    ...
    audit_records = []
    processed_images_for_sheet = []

    for item in PILOT_IMAGES:
        fname = item["file"]
        ...
        # Hardlink image to auto_annotations (or copy if cross-volume)
        ...
        detected_boxes = [] # each: [cid, x_center, y_center, w, h, score, [x1, y1, x2, y2]]
        confidences = []

        if cname == "Healthy_Leaf":
            # Strict rule: Healthy leaves must have 0 bounding boxes
            ...
            # Run inference
            print(f"Detecting candidates for [{cname}] {fname} (prompt: '{prompt}')...")
            ...
            if len(raw_boxes) > 0:
                # Filter out boxes that are the entire image or smaller than 12x12
                valid_idx = []
                for idx, box in enumerate(raw_boxes):
                    bw = box[2] - box[0]
                    bh = box[3] - box[1]
                    # Discard boxes covering > 95% of image or smaller than 12px
                    if bw < 12 or bh < 12:
                        continue
                ...
                if len(valid_idx) > 0:
                    ...
                    keep_idx = apply_nms(filtered_boxes, filtered_scores, iou_thresh=0.40)
            ...
            if n_boxes == 0:
                notes = "WARNING: 0 candidate boxes detected on disease image. Requires manual annotation."
                suspicious = True
            elif n_boxes > 8:
                notes = f"Notice: High candidate count ({n_boxes} boxes). Check for coalesced blotch/spot clutter."
                suspicious = True
            else:
        ...
        processed_images_for_sheet.append({
            "image_path": src_path,
            "filename": fname,
            "class_name": cname,
    ...
    # Save CSV Audit File
    audit_csv = OUT_AUDIT / "pilot_audit.csv"
    ...
    print("\nGenerating 10-image visual inspection contact sheet...")
    generate_contact_sheet(processed_images_for_sheet, OUT_AUDIT / "pilot_contact_sheet.png")

def generate_contact_sheet(items, out_path):
    """Draws boxes and metadata onto a 5x2 grid contact sheet."""
    ...
    try:
        ...
        font_title = font_sub = font_box = font_meta = ImageFont.load_default()

    # Header
    draw.text((margin, margin), "Grounding DINO Candidate Annotation Pilot (10 Test Images)", fill=(255, 255, 255), font=font_title)
    draw.text((margin, margin + 36), "Model: IDEA-Research/grounding-dino-tiny | Status: AUTO_GENERATED (Candidate Proposals Only, NOT Ground Truth)", fill=(160, 175, 195), font=font_sub)

    colors = {
        "Aphids_Disease": (255, 75, 75),   # Red
        "Blotch": (255, 180, 30),          # Amber
        "Leaf_Spot": (0, 210, 255),        # Cyan
        "Healthy_Leaf": (50, 220, 100)     # Green
    }
